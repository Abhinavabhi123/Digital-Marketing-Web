    ...
    <div className=" w-full h-full bg-slate-950 md:pb-20">
      ...
      <div className="w-full min-h-[30rem] p-10 md:p-20">
        <div className="space-y-5">
          ...
          <p
            className="text-sm md:text-base text-gray-500 md:px-10"
            data-aos="fade-up"
          >
      ...
      {!viewDetails ? (
        <div className="w-full min-h-[40rem] px-36">
          <div className="w-full h-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 bg-transparent gap-6">
            {cards.map((item) => (
              <div
                key={item.id}
                className="w-full h-96 bg-transparent ring-4 rounded-lg relative overflow-hidden group cursor-pointer"
              >
                <img
                  src={item.url}
                  alt="portfolio image"
                ...
                />
                <div className="absolute w-full h-full bg-black/50 -bottom-96 transition-all duration-500 group-hover:-bottom-72 p-5 space-y-3">
                  <h1>{item.title}</h1>
                  <button
            ...
            <div className="w-full h-full  bg-transparent flex flex-col md:flex-row pt-10 md:pt-0">
              ...
              <div className="w-full h-full md:w-1/2 p-10 md:p-20 flex justify-center items-center">
                <img
                  src={cards[data].url}
                  alt="person image"
                  className="w-full min-w-36 md:min-w-60 h-full object-contain rounded-lg ring-2"
                />
              </div>
            </div>
          )}
